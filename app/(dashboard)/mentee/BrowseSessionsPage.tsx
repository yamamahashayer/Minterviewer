...
import { Search, Calendar, DollarSign, Filter, Star, Clock, MessageSquare } from 'lucide-react';
...
export default function BrowseSessionsPage() {
    ...
    const handleBookSession = async (slot: TimeSlot) => {
        ...
        try {
            ...
            if (!menteeId) {
                alert("Please log in again to book a session. (User ID missing)");
                setLoading(false);
                return;
            }

            console.log("Booking with Mentee ID:", menteeId);
            ...
            const response = await fetch('/api/stripe/create-checkout-session', {
                ...
                body: JSON.stringify({
                    ...
                    menteeId: menteeId, // Pass mentee ID
        ...
        <div className="p-8 max-w-[1400px] mx-auto min-h-screen">
            ...
            >
                <div className="flex items-center justify-between mb-4">
                    ...
                    <Button
                        onClick={() => setShowNewBookingFlow(!showNewBookingFlow)}
                        className="bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700"
                    >
                        {showNewBookingFlow ? 'Show Time Slots' : 'Show Session Cards'}
                    </Button>
                </div>
            </motion.div>
            ...
            {showNewBookingFlow ? (
                <BookingFlow
                    mentors={mentors}
                    onBack={() => setShowNewBookingFlow(false)}
                />
            ) : (
                <>
                    {/* Filters */}
                                            ...
                                            <div className="text-right">
                                                ...
                                                <div className="flex gap-2">
                                                    <Button
                                                        size="icon"
                                                        variant="ghost"
                                                        className="h-9 w-9 mt-1 text-[var(--foreground-muted)] hover:text-[var(--accent-cyan)] hover:bg-[var(--accent-cyan)]/10"
                                                        onClick={() => router.push(`/mentee?tab=messages&chatWith=${slot.mentor.userId}`)}
